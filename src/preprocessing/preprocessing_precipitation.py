...
# -*- coding: utf-8 -*-
"""
Created on Sun May 31 07:52:20 2026
...
def plot_missing_values(df, column, consecutive_missing, timeframe):
    ...
    for i in range(len(consecutive_missing)):
        ...
        df.loc[plot_start:plot_stop, column[0]].plot(x_compat=True, ax=ax1)
        ax1.xaxis.set_major_formatter(DateFormatter("%Y-%m-%d %H:%M:%S"))
        ax2 = ax1.twinx()
        df.loc[plot_start:plot_stop, column[1]].plot(x_compat=True, ax=ax2)
...
timeframe = '3d'
plot_missing_values(df_result_preprocessed, [column, 'PM_precipitation'], consecutive_missing, timeframe)