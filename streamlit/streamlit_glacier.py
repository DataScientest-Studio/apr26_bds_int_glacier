...
if page == pages[0]:
    ...
    st.subheader('Goal')
    st.write('Setting up a model to compute the water level of the stream Vernagtbach from the meteorological variables:')
    ...
    st.write('The variables were measured at two measurement stations at 2640 m (PM) and 3080 m (SK) in the vicinity of the glacier from January 1, 2013 to December 31, 2024 in steps of 5 minutes.')
    st.subheader('Issues')
    st.write('The problem of the project is not a conventional data science problem:')
    st.markdown('- not a typical regression problem because the time dependence spoils the independence between the observations.')
    st.markdown('- not a typical time series problem because it is not the prediction of the water level from the previous water levels.')

if page == pages[1]: 
    st.header("Data Visualization of a selection of variables")
    
    st.subheader('Water Level (target variable)')
...
if page == pages[4]:
    ...
    st.markdown('- Transformation of variables such as wind and precipitation to ensure a continuous variation with time')
...
if page == pages[5]:
    ...
    st.markdown('- To feed data to a Deep Learning Model a CustomDataset was defined in Pytorch:')
    st.image('plots/time_chunks.png', width='stretch')
    ...
    st.markdown('- For the validation and the output of the model an output of the model is computed by averaging the output chunks of the model on their position of the time axis.')
    ...
    st.image('plots/architecture.png', width='stretch')
    # st.write('Sequential(')
    ...
    st.markdown('- Graphical presentation of the result for the first model:')
    st.image('model_PM_0_result.png', width='stretch')
    
    st.markdown('- Results for 4 different models:')
    results = pd.DataFrame(
        {
        "model_PM_0": [10.74, '1 day'],
        "model_all_0": [9.2, '1 day'],
        "model_pm_1": [8.41, '7 days'],
        "model_all_1": [9.38, '7 days'],
        },
        index=["rmse", "time span"])
    # results = pd.DataFrame(
    #     {
...
if page == pages[6]:
    ...
    st.markdown('- When it works: application to other glaciers and their streams')